...
- `report.html` — human-readable HTML report (if generated by CLI or by calling `report.to_html()`).
- `artifacts/xai_examples/` (optional) — XAI overlay images for selected worst predictions (classification, XAI enabled).
- `artifacts/data_quality/` (optional) — thumbnails or artifacts from data quality analysis, when run.
