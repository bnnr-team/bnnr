...
**1. XAI-guided candidate selection is indistinguishable from random selection.** From scratch on Imagewoof: median Δ = +0.00 pp, 95% CI [−0.20, +0.15], p = 0.945. In the repair regime on Waterbirds: median Δ = +0.39 pp, 95% CI [−0.16, +0.93], ns. Under equal deployed epochs on Waterbirds: median Δ = +0.55 pp, 95% CI [−0.78, +1.48], ns. Three protocols, two datasets, two regimes, always the same answer at n=10.
...
A fourth defect surfaced in the Imagewoof analysis and affected every table the grand-benchmark summarizer had produced: **`summarize_grand.py` computed the rank-biserial effect size as `1 - 2W/(n(n+1))`**, where the matched-pairs statistic is `(W⁺ − W⁻)/(W⁺ + W⁻)`. Because the shipped expression takes `W = min(W⁺, W⁻)`, it equals `(1 + |r|)/2`: the `min` discards the direction, the range is [0.5, 1.0], and it cannot express a null. **No function of the printed value recovers the sign.** This is not a scaling error — it is a different quantity. The headline Imagewoof contrast printed `r = 0.53` next to `p = 0.945`; from the raw per-seed accuracies (W⁺ = 17, W⁻ = 19) the value is **r = −0.06**. A second defect in the same file made the Δ point estimate and its bootstrap CI unpaired — `median(x) − median(y)` where the Wilcoxon p tests `median(x − y)` — which on this contrast produced an interval roughly twice as wide as the paired one.

**Fixed in #398.** Both estimators now live in `benchmarks/stats.py`, imported by both summarizers, and every table in these findings has been regenerated rather than corrected by hand. Do **not** convert old tables with `true r = 2 × printed r − 1`: it recovers only `|r|`, because the defective statistic discarded the sign. The Waterbirds summarizer computed the statistic directly and correctly, so its effect sizes stand. Two of its p-values moved for an unrelated reason — tie detection — and two further published values turned out not to follow from the committed records under any estimator. Both kinds are itemised in `findings_waterbirds.md` §3.2.
...
|---|---|---|---|---|
| T20 Imagewoof, from scratch | equal compute, budget 40 | +0.00 pp | [−0.20, +0.15] | p = 0.945 |
| T20 Imagewoof, from scratch | equal deployed epochs, budget 80 | +0.06 pp | [−0.17, +0.71] | p = 0.461 |
...
On Waterbirds the within-arm contrast — same arm, same selection rule, only the epoch budget differing — gives +1.95 pp (xai) and +3.35 pp (random), 6/8 positive. Nothing survives Holm at n=8 (best adjusted 0.099), so the statement is that the direction is consistent and the sample is the limit. Note that the Waterbirds extension buys equal deployed epochs at **3× the compute**, and is a diagnostic arm rather than a headline.
...
- The faithfulness noise floor was initially unavailable — the base diagnosis and `base_frozen` were expected to be two independent OptiCAM passes over identical weights, but paired |ΔEBPG| had median 0.0002 and maximum 0.0004, suspiciously small for a randomly-initialised optimiser. This was resolved by a forced-independent reproduction (real Waterbirds probe set, N=2000, no caching in the evaluated code path), which reproduced near-identical values (EBPG ≈ 0.0004, IoU@0.5 ≈ 0.00015, Pointing Game ≈ 0.0035) — confirming this is the expected effect of averaging over ~2000 per-image measurements, not a caching defect. **A measured floor is now reported** (see `findings_waterbirds.md` §6.4), and the §III.3 differences exceed it by 5–123× depending on the metric.
...
Fix the group-balanced subsampling defect in the DFR baseline. Compute the mask-coverage baseline so the EBPG dynamic range is known (III.7) — this needs no GPU and the artifacts are already on disk. Report the OptiCAM batch-dependence upstream. Correct the AICD description in `benchmarks/README.md` and state, for each augmentation, the condition under which it is the right intervention. ~~Fix the rank-biserial formula in `summarize_grand.py`~~ — **done in #398**, but not as this line proposed. It recommended `1 - 4W/(n(n+1))` "signed by the median difference", and both halves are wrong: that expression is exactly `|r|`, and `r` is signed by `W⁺ − W⁻`, not by the median (ranks carry magnitude, the median only counts — `d = [1, 2, 3, −100, −101]` has median +1 and r = −0.20). The shipped fix computes `(W⁺ − W⁻)/(W⁺ + W⁻)` directly. The `2r − 1` conversion this line proposed is likewise retired: it expands to `1 - 4W/(n(n+1))` and recovers only the magnitude, so the affected tables were regenerated rather than converted.

---

# Part V — What we can and cannot claim

**Can claim.** XAI-guided selection shows no detectable advantage over random at n=10, across two datasets, two regimes and three protocols, with CIs that exclude effects larger than roughly 1 pp on Imagewoof and 1–2 pp on Waterbirds. On Imagewoof the effect size is r = −0.06 (the summarizer printed 0.53 until #398; see Part II). The equal-compute deficit against plain baselines is dominated by the epoch split. ICD and AICD dissociate on the two measured axes, with complete separation on EBPG under randomised assignment. The diagnosed base reproduces the literature's broken ERM model. OptiCAM is batch-size-dependent by construction. A measured faithfulness noise floor now exists (§III.7, `findings_waterbirds.md` §6.4): the §III.3 differences between ICD, AICD and ChurchNoise exceed it by 5–123× depending on the metric, so those differences are not measurement noise.

**Cannot claim.** That XAI guidance has *no* effect — only that none is detectable at this power, with a Waterbirds detectability floor near 0.5 pp set by group quantisation. That the ICD/AICD dissociation generalises beyond Waterbirds — it is exploratory, n = 12 versus 6, one dataset, and the mechanism is unexplained. That BNNR is competitive with published spurious-correlation methods — it is not, in absolute terms (III.6). Anything about ChurchNoise, at n = 2 and with divergent CPU/GPU paths.
...
- `summarize_grand.py` computed rank-biserial as `1 - 2W/(n(n+1))` with `W = min(W⁺, W⁻)`; the matched-pairs statistic is `(W⁺ − W⁻)/(W⁺ + W⁻)`. The shipped expression equals `(1 + |r|)/2`: range [0.5, 1.0], no null, direction discarded by the `min`. Its Δ and bootstrap CI were also unpaired. Both fixed in #398 (`benchmarks/stats.py`); tables regenerated. `true r = 2 × printed r − 1` expands to `1 - 4W/(n(n+1))`, which is exactly `|r|` — it recovers the magnitude and nothing else. Regenerate, do not convert.
